...
# Get the actual mount point (may be suffixed)
get_actual_mount_point() {
  mount | grep -E "/$ENC_SHARE_NAME on " | sed -E 's|.* on (/Volumes/[^ ]+) .*|\1|' | head -1
}
...
main() {
  ...
  if is_share_mounted_anywhere; then
    ...
    echo "$(date -Iseconds) Found mount at $actual_mp, fixing to $MOUNT_POINT" >&2
    diskutil unmount "$actual_mp" >/dev/null 2>&1
    sleep 1
    preclean_mount_dirs
